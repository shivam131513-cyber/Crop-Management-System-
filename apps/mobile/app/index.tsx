...
const LANGUAGES: Array<{ code: 'pa' | 'hi' | 'en'; label: string; greeting: string }> = [
  { code: 'pa', label: 'PA', greeting: 'ਸਤਿ ਸ੍ਰੀ ਅਕਾਲ 🙏' },
  { code: 'hi', label: 'HI', greeting: 'नमस्ते 🙏' },
  { code: 'en', label: 'EN', greeting: 'Hello 👋' },
];
...
export default function HomeScreen() {
  const { profile, isOnline, setProfile } = useAppStore();
  const slot = getCurrentSlot();
  const [fadeAnim] = useState(new Animated.Value(0));
  const [langConfirmAnim] = useState(new Animated.Value(0));
  const [langConfirmText, setLangConfirmText] = useState('');
  ...
  const currentLangIdx = LANGUAGES.findIndex((l) => l.code === profile.language) ?? 0;
  const currentLang = LANGUAGES[currentLangIdx] ?? LANGUAGES[0];

  const handleLangToggle = () => {
    const nextIdx = (currentLangIdx + 1) % LANGUAGES.length;
    const nextLang = LANGUAGES[nextIdx];
    setProfile({ ...profile, language: nextLang.code });
    setLangConfirmText(nextLang.label);

    // Flash confirmation animation
    Animated.sequence([
      Animated.timing(langConfirmAnim, { toValue: 1, duration: 150, useNativeDriver: true }),
      Animated.delay(700),
      Animated.timing(langConfirmAnim, { toValue: 0, duration: 200, useNativeDriver: true }),
    ]).start();
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <Animated.View style={[styles.header, { opacity: fadeAnim }]}>
          <View>
            <Text style={styles.greeting}>{currentLang.greeting}</Text>
          ...
          <View>
            <TouchableOpacity
              ...
              accessibilityHint="Cycles between Punjabi, Hindi and English"
            >
              <Ionicons name="language" size={13} color="#fff" style={{ marginRight: 4 }} />
              <Text style={styles.langText}>{currentLang.label}</Text>
            </TouchableOpacity>
            {/* Flash confirmation */}
            <Animated.View
              style={[styles.langConfirm, { opacity: langConfirmAnim }]}
              pointerEvents="none"
            >
              <Text style={styles.langConfirmText}>{langConfirmText} ✓</Text>
            </Animated.View>
          </View>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  langConfirm: {
    ...
    backgroundColor: '#1a1a2e', paddingHorizontal: 8, paddingVertical: 3,
    borderRadius: radius.sm, borderWidth: 1, borderColor: colors.primary,
  },
  langConfirmText: { color: colors.accent, fontSize: 11, fontWeight: '600' },
  offlineBanner: {
    flexDirection: 'row', alignItems: 'center',
